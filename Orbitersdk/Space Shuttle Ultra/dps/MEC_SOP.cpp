...
namespace dps
{
	...
	{
		pMEC1 = static_cast<MasterEventsController*>(STS()->psubsystems->GetSubsystemByName( "MEC1" ));
		assert( (pMEC1 != NULL) && "MEC_SOP::Realize.pMEC1" );
		pMEC2 = static_cast<MasterEventsController*>(STS()->psubsystems->GetSubsystemByName( "MEC2" ));
		assert( (pMEC2 != NULL) && "MEC_SOP::Realize.pMEC2" );
		return;
	}

	void MEC_SOP::OnPostStep( double SimT, double DeltaT, double MJD )
	{
