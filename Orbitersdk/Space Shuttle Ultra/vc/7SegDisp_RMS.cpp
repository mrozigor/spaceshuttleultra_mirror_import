...
namespace vc
{
	...
	void _7SegDisp_RMS::Realize( void )
	{
		...
		for (int i = 0; i < 8; i++) LED_JointSelect[i].Connect( pBundle, 7 - i );
	...
	void _7SegDisp_RMS::Controller( void )
	{
		...
		{
			if (LED_ParameterSelect[0].IsSet())
			{
			...
			else if (LED_ParameterSelect[4].IsSet())
			{
				...
				else// if (LED_JointSelect[7].IsSet())
				{
					// SHOULDER YAW
					char cbuf[8];
					int decimalpoint = 6;
			...
			else// if (LED_ParameterSelect[7].IsSet())
			{
				// TEST
