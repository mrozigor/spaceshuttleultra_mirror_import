...
namespace vc
{
	...
	void MDU::OMSMPS( oapi::Sketchpad2* skp )
	{
		...
		skp->SetFont( skpSSUAFont_h10bold );
		...
		for (nPos = 0; nPos < 2; nPos++)
		{
			dNum = STS()->GetPropellantMass( STS()->oms_helium_tank[nPos] ) * 228.6;
			sprintf_s( cbuf, 16, "%04.0f", dNum );
			skp->Text( 56 + 52 * nPos, 45 + nPos, cbuf, strlen( cbuf ) );
			if (dNum >= 1500)
			{
	...
	void MDU::SPI( HDC hDC )// 304, 305, 602, 603, 801, 901
	{
		...
		// elevons
		dNum = range( -35, STS()->aerosurfaces.leftElevon, 20 );
		...
		dNum = range( -35, STS()->aerosurfaces.rightElevon, 20 );
		nPos = Round( 237 + (4 * dNum) );
		tri[0].x = 101;
		tri[0].y = nPos;
		tri[1].x = 117;
		...
		// body flap
		dNum = range( -11.7, STS()->aerosurfaces.bodyFlap, 22.5 );
		...
		// rudder
		dNum = range( -30, STS()->aerosurfaces.rudder, 30 );
		...
		// aileron
		dNum = range( -5, (STS()->aerosurfaces.leftElevon - STS()->aerosurfaces.rightElevon) * 0.5, 5 );
		...
		// spedbrake
		SetTextColor( hDC, CR_YELLOW );
		dNum = range( 0, STS()->aerosurfaces.speedbrake, 100 );
