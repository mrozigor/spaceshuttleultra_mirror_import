...
namespace eps 
{
	PRSD::PRSD( int internalsets, bool EDOhw, int pallets, AtlantisSubsystemDirector* _director ):AtlantisSubsystem( _director, "PRSD" )
	{
		assert( (internalsets >= 2) && (internalsets <= 5) && "PRSD::PRSD.internalsets" );
		assert( (pallets >= 0) && (pallets <= 2) && "PRSD::PRSD.pallets" );

		this->sets = internalsets + (pallets * 4);
		this->EDOhw = EDOhw;
	...
	void PRSD::Realize( void )
	{
		...
		pBundle = STS()->BundleManager()->CreateBundle( "PRSD_O2HtrAssyTemp2", 16 );
		for (int i = 0; i < sets; i++) O2TKhtrassytemp2[i].Connect( pBundle, i );

		pBundle = STS()->BundleManager()->CreateBundle( "PRSD_H2HtrAssyTemp", 16 );
		...
		pBundle = STS()->BundleManager()->CreateBundle( "PRSD_O2FluidTemp", 16 );
		for (int i = 0; i < sets; i++) O2TKtemp.at( i ).Connect( pBundle, i );
		
		pBundle = STS()->BundleManager()->CreateBundle( "PRSD_H2FluidTemp", 16 );
		...
		pBundle = STS()->BundleManager()->CreateBundle( "PRSD_O2Press2", 16 );
		...
		pBundle = STS()->BundleManager()->CreateBundle( "PRSD_H2Press2", 16 );
		for (int i = 0; i < sets; i++) H2TKhtrcntrpress.at( i ).Connect( pBundle, i );
		return;
	}

	void PRSD::OnPreStep( double fSimT, double fDeltaT, double fMJD )
	{
		vman.OnPostStep( fSimT, fDeltaT, fMJD );
	...
	double PRSD::GetSubsystemEmptyMass() const
	{
		double mass = (EDO_PALLET_DRY_MASS * pallets) + ((O2_TANK_DAY_MASS + H2_TANK_DAY_MASS) * sets);

		if (EDOhw) mass += EDO_KIT_MASS;
