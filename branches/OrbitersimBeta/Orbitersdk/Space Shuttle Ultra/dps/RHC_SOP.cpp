...
namespace dps
{
	...
	void RHC_SOP::Realize( void )
	{
		...
		AftSense.Connect( pBundle, 0 );// HACK should first go to switch RM

		pRHC_RM = static_cast<RHC_RM*>(FindSoftware( "RHC_RM" ));
		assert( (pRHC_RM != NULL) && "RHC_SOP::Realize.pRHC_RM" );
		return;
	}

	void RHC_SOP::OnPostStep( double SimT, double DeltaT, double MJD )
	{
		switch (GetMajorMode())
		{
			case 304:
