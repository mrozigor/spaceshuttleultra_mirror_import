...
VideoControlUnit::VideoControlUnit( AtlantisSubsystemDirector* _director, const string& _ident ):AtlantisSubsystem( _director, _ident )
{
	outsel = OUT_MON1;
...
void VideoControlUnit::Realize( void )
{
	...
	for (int i = 32; i < 43; i++) input[i].Connect( pBundle, i - 32 );
	...
	for (int i = 16; i < 26; i++) output[i + CameraPowerA_TB].Connect( pBundle, i - 16 );
...
bool VideoControlUnit::OnParseLine( const char* line )
{
	...
	{
		sscanf_s( line + 12, "%d %d %d %d %d", &camerapowerA, &camerapowerB, &camerapowerC, &camerapowerD, &camerapowerRMS );
		return true;
	}
...
void VideoControlUnit::OnPreStep( double fSimT, double fDeltaT, double fMJD )
{
	// input
	...
	else if (input[VideoInputMUX1].IsSet()) outsel_in[outsel] = IN_MUX1;
	...
	if ((STS()->VCMode >= VC_PLBCAMFL) && (STS()->VCMode <= VC_RMSCAM))
	{
		// "man" control
		if (ManPanLeft.IsSet())
		{
			left[STS()->VCMode - VC_PLBCAMFL] = true;
		}
		else if (ManPanRight.IsSet())
		{
			right[STS()->VCMode - VC_PLBCAMFL] = true;
		}

		if (ManTiltUp.IsSet())
		{
			up[STS()->VCMode - VC_PLBCAMFL] = true;
		}
		else if (ManTiltDown.IsSet())
		{
			down[STS()->VCMode - VC_PLBCAMFL] = true;
		}
	}
	else
	{
